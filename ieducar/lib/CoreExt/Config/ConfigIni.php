...
class ConfigIni extends Config
{
	...
	* @var string
	*/
	const CONFIG_INI_INHERITANCE_SEP = ':';

	/**
	* Caractere de namespace das diretivas do arquivo ini.
	* @var string
	*/
	const CONFIG_INI_NAMESPACE_SEP   = '.';
	...
	*/
	...
	/**
	* Construtor.
	* @param  $filename string  Caminho para o arquivo ini
	* @param  $section  string  Seção desejada para o carregamento das configurações
	*/
	...
	*/
	private function loadFile($filename)
	{
		...
		// que use um método da classe Config.
		...
		*/
		...
		foreach ($config as $section => $data)
		{
			...
			if (FALSE !== strpos($section, self::CONFIG_INI_INHERITANCE_SEP)) {
				$sections = explode(self::CONFIG_INI_INHERITANCE_SEP, $section);
	...
	*/
	private function processSection(array $data)
	{
		...
		foreach ($entries as $key => $value) {
			if (FALSE !== strpos($key, self::CONFIG_INI_NAMESPACE_SEP)) {
				$keys = explode(self::CONFIG_INI_NAMESPACE_SEP, $key);
			} else {
				$keys = (array) $key;
			}
