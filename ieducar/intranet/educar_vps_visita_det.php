...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
			...
			if($ref_cod_vps_entrevista)
			{
				...
				if(class_exists("clsPessoaFj"))
				{
					...
					if(is_string($endereco))
					{
						...
						$this->addDetalhe(array("Endereço Empresa", "{$endereco}"));
					}
		...
		{
			$this->addDetalhe(array("Responsável Visita", "{$registro["nm_usuario"]}"));
		}
		...
		if(is_numeric($registro["avaliacao"]))
		{
			...
			$this->addDetalhe(array("Avaliação de VPS?", "{$avaliacao}"));
		}
