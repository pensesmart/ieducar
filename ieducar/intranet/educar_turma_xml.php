...
//		$db->Consulta( "SELECT cod_turma, nm_turma FROM pmieducar.turma WHERE ref_ref_cod_escola = {$_GET["esc"]} AND ref_ref_cod_serie = {$_GET["ser"]} AND ativo = 1 ORDER BY nm_turma ASC" );
		...
		$db = new clsBanco();
		$db->Consulta( "SELECT cod_turma, nm_turma || ' - ' || ano AS nome FROM pmieducar.turma WHERE ref_cod_instituicao = {$_GET["ins"]} AND ref_cod_curso = {$_GET["cur"]} AND ref_ref_cod_escola is null AND ref_ref_cod_serie is null AND ativo = 1 ORDER BY nm_turma ASC" );
		while ( $db->ProximoRegistro() )
		{
			list( $cod, $nome ) = $db->Tupla();
			echo "	<turma cod_turma=\"{$cod}\">{$nome}</turma>\n";
